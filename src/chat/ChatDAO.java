...
//이상할때
	/*
	 * public ArrayList<ChatDTO> getChatListByID(String fromID, String toID, String
	 * chatID, int number){
	 */
...
//잘못되면 바꾸기
	/*
	 * public ArrayList<ChatDTO> getChatListByRecent(String fromID, String toID,
	 * String chatID, int number){
	 */
		...
		try {
			...
			for(int i = 0; i< chatList.size(); i++) {
				...
				for(int j = 0; j< chatList.size(); j++) {
					...
					if(x.getFromID().contentEquals(y.getToID())&& x.getToID().equals(y.getFromID())){
	...
	public int getAllUnreaedChat(String userID) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			pstmt.setString(1, userID);
			rs = pstmt.executeQuery();
			if(rs.next()) {//안읽은 메시지 개수
				return rs.getInt("COUNT(chatID)");
			}
			return 0;//그렇지 않을 경우 받은 메시지  0임
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
